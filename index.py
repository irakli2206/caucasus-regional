...
shapefiles = [
    'georgia_data/geo_admbnda_adm1_geostat_20191018.shp', 
    'armenia_data/arm_admbnda_adm1_2019.shp'
]

# Read and concatenate shapefiles into one GeoDataFrame
world = gpd.GeoDataFrame(pd.concat([gpd.read_file(shp) for shp in shapefiles], ignore_index=True))

print('world', world['ADM0_EN'])

# Create a plot
fig, ax = plt.subplots(figsize=(15, 10))

# Plot all geometries with no fill and black edge color
world.plot(ax=ax, edgecolor='gray', facecolor='none', linewidth=0.3)

# Plot the external boundary of the entire geometry
world.dissolve().boundary.plot(ax=ax, color='gray', linewidth=0.3)

plt.title('Geographical Boundaries of Georgia and Armenia')
